import Footer from '../components/Footer.jsx';

function PrivacyPolicy() {
    return (
        <div className="min-h-screen flex flex-col bg-[#F3F2EF] font-sans">
            <main className="flex-1 px-4 py-12 sm:py-16">
                <div className="max-w-4xl mx-auto">
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sm:p-10">
                        <p className="text-sm font-semibold uppercase tracking-[0.2em] text-[#0A66C2]">
                            Privacy Policy
                        </p>
                        <h1 className="mt-3 text-3xl sm:text-4xl font-bold text-slate-900">
                            Your data, your control.
                        </h1>
                        <p className="mt-4 text-slate-600 leading-7">
                            This demo application stores user data for profile,
                            networking, content, and notification features. It
                            should be treated as a development project, not a
                            production privacy implementation.
                        </p>
                        <p className="mt-4 text-slate-600 leading-7">
                            In a real deployment, this page would explain data
                            collection, cookie usage, account activity, uploads,
                            and retention in more detail.
                        </p>
                    </div>
                </div>
            </main>

            <Footer />
        </div>
    );
}

export default PrivacyPolicy;
